...
for t in range(1, T + 1):
    N = int(input())  # N: 신청서 수
    work_times = sorted([tuple(map(int, input().split())) for _ in range(N)], key=lambda x: x[1])

    e = work_times[0][0]  # 가장 종료시간이 이른 0번째거를 e로 설정
    cnt = 1  # 0번째거는 이미 작업됐으므로 1로 초기화

    for i in range(1, N):  # 1번째~마지막
        if work_times[i][0] >= e:  # 해당 신청서 시작시간이 종료시간 보다 같거나 클때
            e = work_times[i][1]  # 종료시간은 해당 신청서 종료시간으로 업데이트
            cnt += 1  # 작업 처리 개수 +1
