            ...
            if not mapp2[x][y]:  # 맵이 0이면 블록이 없다는 뜻이기에 다음 행 탐색
                ...
                while q:  # BFS // 가능한 벽돌 다 터뜨림
                    ...
                    for k in range(1, qq[2]):  # 블록 숫자 - 1만큼 연쇄폭발
                        for i in range(4):
                            if 0 <= qq[0] + dx[i] * k < H and 0 <= qq[1] + dy[i] * k < W and \
                                    mapp2[qq[0] + dx[i] * k][qq[1] + dy[i] * k] != 0:
                                if (qq[0] + dx[i] * k, qq[1] + dy[i] * k, mapp2[qq[0] + dx[i] * k][
                                    qq[1] + dy[i] * k]) not in q:  # 큐에 중복으로 들어가는 것 방지
                                    q.append((qq[0] + dx[i] * k, qq[1] + dy[i] * k,
                                              mapp2[qq[0] + dx[i] * k][qq[1] + dy[i] * k]))
