...
for i in range(E):
    ...
    if not tree[p][0]:      # parent 노드의 왼쪽 자식이 없다면
        tree[p][0] = c      # 왼쪽 자식으로 넣고
    else:                   # parent 노드 왼쪽 자식이 있다면
        tree[p][1] = c      # 오른쪽 자식으로 넣자
    tree[c][2] = p          # 이후 부모 노드 초기화
